...
export function LiquidButton({
  ...
  const inner = (
    <>
      {/* Liquid fill black badge that expands */}
      <span
        ref={fillRef}
        aria-hidden
      ...
      <span className="relative z-10 flex items-center gap-3 py-1.5 pl-1.5 pr-6">
        ...
        <span
          ref={labelRef}
          className="text-[0.9375rem] font-semibold tracking-tight text-[#0a0a0a] transition-colors"
        >
          {children}
        </span>
      </span>
  ...
  const shared =
    `group relative inline-flex select-none items-center ${radius} ring-2 ring-transparent transition-all duration-300 hover:ring-white/80 active:ring-white/80 shadow-[0_10px_30px_-10px_rgba(0,0,0,0.6)] ` +
    className;
